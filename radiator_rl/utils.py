...
def plot_model(
    ...
    """Plot a single model with professional styling."""
    
    # Professional color scheme
    colors = {
        'model': '#2E86AB',         # Professional blue
        'T_out': '#6C757D',         # Gray
        'target': '#06A77D',        # Teal
    ...
    if owner_schedule:
        min_step = min(model_data['time'])
        max_step = max(model_data['time'])
    ...
    # Temperature lines
    ax[0][0].plot(model_data['time'], model_data['T_out'], 
                  label='T_out', color=colors['T_out'], linestyle='--', 
                  linewidth=2, alpha=0.7, zorder=1)
    ax[0][0].plot(model_data['time'], model_data['T_in'], 
                  label=f'T_in {model_name}', color=colors['model'], 
    ...
    ax[0][0].set_xlim([min(model_data['time']), max(model_data['time'])])
    ax[0][0].set_title('Temperature Over Time', fontsize=13, fontweight='bold', 
                       color=colors['text'], pad=15)
    ax[0][0].tick_params(colors=colors['text'])
    
    # --- Energy Consumption Plot ---
    ax[0][1].plot(model_data['time'], energy_cumsum, 
                  label=f'{model_name}', color=colors['model'], linewidth=2.5)
    ...
    ax[0][1].set_title('Energy Consumption', fontsize=13, fontweight='bold', 
                       color=colors['text'], pad=15)
    ...
    ax[1][0].set_title('Cumulative Reward', fontsize=13, fontweight='bold', 
                       color=colors['text'], pad=15)
    ax[1][0].tick_params(colors=colors['text'])
    
    # Statistics box
    final_reward = reward_cumsum[-1]
    text_str = f'Total: {final_reward:.2f}'
    ax[1][0].text(0.03, 0.05, text_str, transform=ax[1][0].transAxes, 
                  fontsize=10, verticalalignment='bottom', bbox=box_props, 
                  color=colors['text'])
    
    # --- Energy Cost Plot ---
    ax[1][1].plot(model_data['time'], cost_cumsum, 
                  label=f'{model_name}', color=colors['model'], linewidth=2.5)
    ax[1][1].set_xlabel(f'Time (every {dt/60:.0f} min)', fontsize=11, color=colors['text'])
    ax[1][1].set_ylabel('Cumulative Cost (CHF)', fontsize=11, color=colors['text'])
    ax[1][1].legend(loc='lower right', framealpha=0.95, fontsize=10)
    ax[1][1].set_title('Energy Cost', fontsize=13, fontweight='bold', 
                       color=colors['text'], pad=15)
    ax[1][1].tick_params(colors=colors['text'])
    
    # Statistics box
    final_cost = cost_cumsum[-1]
    text_str = f'Total: CHF{final_cost:.2f}'
...
def compare_models(
    ...
    text_str = f'{model1_name}: CHF{final_cost1:.2f}\n{model2_name}: CHF{final_cost2:.2f}\nSavings: {cost_savings_pct:.1f}%'
    ax[1][1].text(0.03, 0.97, text_str, transform=ax[1][1].transAxes, 
                  fontsize=10, verticalalignment='top', bbox=box_props, 
                  color=colors['text'])
